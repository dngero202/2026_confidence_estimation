...
class PseudoLabelStorage:
    ...
    def __init__(self):
        self.local_pseudo = {}
        self.global_pseudo = {}  # Store but not used in LOCAL experiment
    ...
    def get_agreed_samples(self):
        """For LOCAL experiment: return LOCAL pseudo-labels as 'agreed'"""
        indices = list(self.local_pseudo.keys())
        labels = [self.local_pseudo[idx] for idx in indices]
...
def compute_pseudo_label_statistics(pseudo_storage, true_labels):
    """Compute statistics for LOCAL pseudo-labels only"""
    ...
    for idx in range(len(true_labels)):
        ...
        if actual_idx in pseudo_storage.local_pseudo:
...
def main():
    ...
    print("\n" + "="*70)
    print("TRAINING START - LOCAL PSEUDO-LABELS")
    ...
    for epoch in range(1, 301):
        ...
        if use_unlabeled:
            ...
            # ✓ THIS IS THE CORRECT LOCATION (inside main, inside loop)
            if len(pseudo_storage.local_pseudo) > 0:
                stats = compute_pseudo_label_statistics(pseudo_storage, true_unlabeled_labels)
                print(f"\n{'='*70}")
                print(f"EPOCH {epoch}: PSEUDO-LABEL STATISTICS (LOCAL ONLY)")
                print(f"{'='*70}")
                print("FINAL MATCH COUNT (Model = Local):", match_count_total)
                print(f"Total unlabeled samples: {stats['total']}")
                print(f"Local correct: {stats['local_correct']}")
                print(f"Local wrong: {stats['local_wrong']}")
