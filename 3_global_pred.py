...
class PseudoLabelStorage:
    """Store BOTH local and global pseudo-labels"""

    def __init__(self):
        self.local_pseudo = {}  # Store but not used in GLOBAL experiment
        self.global_pseudo = {}

    def update(self, unlabeled_indices, global_labels, local_labels):
        """Store BOTH pseudo-labels by index"""
    ...
    def get_agreed_samples(self):
        """For GLOBAL experiment: return GLOBAL pseudo-labels as 'agreed'"""
        indices = list(self.global_pseudo.keys())  # ← CHANGED: Use global
        labels = [self.global_pseudo[idx] for idx in indices]  # ← CHANGED: Use global
        return np.array(indices), np.array(labels)

    def reset(self):
        """Reset stored pseudo-labels"""
        self.global_pseudo = {}
        self.local_pseudo = {}
...
def compute_pseudo_label_statistics(pseudo_storage, true_labels):
    """Compute statistics for GLOBAL pseudo-labels only"""
    ...
    for idx in range(len(true_labels)):
        ...
        if actual_idx in pseudo_storage.global_pseudo:  # ← CHANGED: Use global
            global_pred = pseudo_storage.global_pseudo[actual_idx]  # ← CHANGED: Use global
            
            if global_pred == true_label:
                global_correct += 1
            else:
                global_wrong += 1
    ...
    return {
        'total': total_samples,
        'global_correct': global_correct,  # ← CHANGED: global_correct
        'global_wrong': global_wrong,      # ← CHANGED: global_wrong
        'global_accuracy': 100.0 * global_correct / total_samples if total_samples > 0 else 0.0,  # ← CHANGED
    }
...
def main():
    ...
    for epoch in range(1, 301):
        ...
        if use_unlabeled:
            ...
            if len(pseudo_storage.global_pseudo) > 0:  # ← CHANGED: Check global_pseudo
                stats = compute_pseudo_label_statistics(pseudo_storage, true_unlabeled_labels)
                print(f"\n{'='*70}")
                print(f"EPOCH {epoch}: PSEUDO-LABEL STATISTICS (GLOBAL ONLY)")  # ← CHANGED
                print(f"{'='*70}")
                print("FINAL MATCH COUNT (Model = Global):", match_count_total)  # ← CHANGED
                print(f"Total unlabeled samples: {stats['total']}")
                print(f"Global correct: {stats['global_correct']}")  # ← CHANGED
                print(f"Global wrong: {stats['global_wrong']}")      # ← CHANGED
                print(f"Global Accuracy: {stats['global_accuracy']:.2f}%")  # ← CHANGED
                print(f"{'='*70}\n")
