...
from loss_local_global import compute_kld_labeled, compute_kld_unlabeled
...
# =========================
# Local Pseudo-Labeling with Nyström
# =========================
def generate_local_pseudo_labels_nystrom(accumulator, k_neighbors=70, n_landmarks=500,
                                         alpha=0.5, num_classes=10, max_iter=50):
    data = accumulator.get_accumulated_data()
    correct_embeddings = data['correct_embeddings']
    correct_labels = data['correct_labels']
    unlabeled_embeddings = data['unlabeled_embeddings']
    ...
    print(f"    Nyström: n_total={n_total}, n_landmarks={n_landmarks}")
    
    # Step 1: Sample landmarks
    landmark_indices = np.random.choice(n_total, size=n_landmarks, replace=False)
    landmark_embeddings = all_embeddings[landmark_indices]
    
    sigma_squared = 1.25
    
    # Step 2: Compute K_nm
    K_nm = np.zeros((n_total, n_landmarks))
    for i in range(n_total):
        for j in range(n_landmarks):
            dist = np.linalg.norm(all_embeddings[i] - landmark_embeddings[j])
            K_nm[i, j] = np.exp(-dist**2 / (2 * sigma_squared))
    
    # Step 3: Compute K_mm (FIXED)
    K_mm = np.zeros((n_landmarks, n_landmarks))
    for i in range(n_landmarks):
        for j in range(n_landmarks):
            dist = np.linalg.norm(landmark_embeddings[i] - landmark_embeddings[j])
            K_mm[i, j] = np.exp(-dist**2 / (2 * sigma_squared))
    
    # Step 4: Invert K_mm
    try:
        K_mm_inv = np.linalg.inv(K_mm + 1e-6 * np.eye(n_landmarks))
    except np.linalg.LinAlgError:
        K_mm_inv = np.linalg.pinv(K_mm)
    
    # Step 5: Compute degree (FIXED)
    K_approx_row_sums = (K_nm @ K_mm_inv @ K_nm.T).sum(axis=1)
    d_approx = np.maximum(K_approx_row_sums, 1e-10)
    d_inv_sqrt = 1.0 / np.sqrt(d_approx)
    
    # Step 6: Initialize labels
    ...
    # Step 7: Label propagation (FIXED)
    for iteration in range(max_iter):
        # S @ F = D^(-1/2) @ K @ D^(-1/2) @ F
        temp1 = d_inv_sqrt[:, None] * F
        temp2 = K_nm @ K_mm_inv @ (K_nm.T @ temp1)
        S_F = d_inv_sqrt[:, None] * temp2
        
        F_new = alpha * S_F + (1 - alpha) * Y_init
        
        diff = np.linalg.norm(F_new - F, 'fro')
        F = F_new
        
        if diff < 1e-6:
            print(f"    Nyström converged at iteration {iteration+1}")
            break
...
def phase2_accumulation_and_pseudo_labels(labeledloader, unlabeledloader, model, 
    ...
    print(f"\n{'='*70}")
    print(f"PHASE 2: ACCUMULATION + PSEUDO-LABEL GENERATION (Nyström)")
    print(f"{'='*70}\n")
    ...
    print("\nStep 4: Generating LOCAL pseudo-labels (Nyström)...")
    local_pseudo = generate_local_pseudo_labels_nystrom(
        accumulator, 
        k_neighbors=args.k_neighbors, 
        n_landmarks=args.n_landmarks,
        alpha=args.alpha_propagation,
        num_classes=10, 
        max_iter=50
    )
    ...
    if len(global_pseudo) > 0 and len(local_pseudo) > 0:
        ...
        print(f"  Agreed samples: {len(agreed_indices)}")
...
def main():
    ...
    print("\n" + "="*70)
    print("TRAINING START - 3-PHASE WITH NYSTRÖM LOCAL METHOD")
    print(f"Label size: {args.label_size}")
    print(f"Warmup epochs: {args.warmup_epochs}")
    print(f"Nyström landmarks: {args.n_landmarks}")
    print("="*70 + "\n")
    ...
    for epoch in range(1, 301):
        ...
        if use_unlabeled:
            print(f"\n{'='*70}")
            print(f"EPOCH {epoch}: 3-PHASE TRAINING (Nyström)")
            print(f"{'='*70}\n")
            ...
            if len(pseudo_storage.global_pseudo) > 0:
                ...
                print(f"{'='*70}")
                print("FINAL COUNT (Local = Global used for training):", match_count_total)
